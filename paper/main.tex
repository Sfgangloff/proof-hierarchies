...
\PLACEHOLDER{headline result: the rough/fine binary granularity yields a
measurable held-out \texttt{pass@}$k$ difference, with the effect growing as
model size decreases. The 21-module Stage 0d sample (1{,}715 real user
theorems) shows depth $\leq 3$ in the dep poset and $\pi_{\mathrm{root}}$
vs.~$\pi_{\mathrm{leaf}}$ size variation ${\approx}22\%$; a multi-level
``intermediate optimum'' is not supported by the data and is not claimed.}
\end{abstract}
